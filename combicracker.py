...
def total_permutations(n, separators_count):
    """Calculate total permutations of lengths 1..n, multiplied by separator options."""
    total = 0
    for r in range(1, n+1):
        total += math.perm(n, r) * separators_count
    return total
...
def main():
    parser = argparse.ArgumentParser(description="Brute-force permutations of input strings to match given hashes (algo guessed from hash length).")
    parser.add_argument("--input-strings", required=True, help="Path to file with possible input strings (one per line)")
    parser.add_argument("--hashes", required=True, help="Path to file with target hashes (one per line, lowercase hex)")
    parser.add_argument("--verbose", action="store_true", help="Display all attempts")
    ...
    separators = ["", " ", "-", "_", ":", "|"]
    total_attempts = total_permutations(len(input_strings), len(separators)) * max(len(a) for a in hash_algo_map.values() or [1])
    ...
    print(f"[+] Estimated total permutations to attempt: {total_attempts:,}")

    # Show detected algorithms even if not verbose
            ...
            for sep in separators:
                ...
                for thash, algos in hash_algo_map.items():
                    for algo in algos:
                        ...
                        if args.verbose:
                            # Show the actual hashed input string
                            print(f"[ATTEMPT {attempt_counter}/{total_attempts}] Algorithm:{algo} | Separator='{sep}' | Input:'{candidate}' | Hash={h.hexdigest()}")
                        else:
                            ...
                            sys.stdout.write(f"\r[Progress] Attempt {attempt_counter}/{total_attempts} ({percent:.2f}%) | ETA: {format_eta(eta)}")
                            sys.stdout.flush()

                        if h.hexdigest() == thash:
                            if not args.verbose:
                                print()  # Move to new line so match isn't on same line
